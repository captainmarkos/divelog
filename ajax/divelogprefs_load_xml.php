...
if(!$res) { echo "ERROR: 1: Query Failed: error-> " . $dbconn->error; exit(); }

if($res->num_rows < 1) {
    // No record was found so let's create an initial record.
    $sqlnew = "SELECT id FROM users WHERE email=?";
    $sqlnew = $db_helper->construct_secure_query($sqlnew, $email);
    $res = $dbconn->query($sqlnew);
    if($res->num_rows > 0) {
        $row = $res->fetch_assoc();
        $user_id = $row['id'];
        $sqlnew = "INSERT INTO divelogprefs (user_id) VALUES (?)";
        $sqlnew = $db_helper->construct_secure_query($sqlnew, $user_id);
        $res = $dbconn->query($sqlnew);
        if(!$res) { echo "ERROR: Insert Failed: error-> " . $dbconn->error; exit(); }
    }
    // Now re-query the table to get the data so we can build the xml.
    $res = $dbconn->query($sql);
    if(!$res) { echo "ERROR: 2: Query Failed: error-> " . $dbconn->error; exit(); }
}

if($res->num_rows < 1) { echo "ERROR: No divelogprefs records found for email='$email'"; exit(); }
$row = $res->fetch_assoc();
